...
public class SkuDetailServiceImpl implements SkuDetailService {
    ...
    //    2.判断 缓存中有没有
        ...
        //    3.缓存没有
        //    4.先问布隆 ，是否有这个商品
            boolean contain=cacheOpService.bloomContains(skuId);
            if (!contain){
            //    5.布隆说没有，一定没有
                log.info("[{}]商品-布隆判定没有，存在隐藏的攻击风险。。。"+skuId);
                return null;
            }
        //    6.布隆说有，有可能有，就需要回源查询数据
            boolean lock=cacheOpService.tryLock(skuId);//为当前商品加自己的分布式锁，eg:100w的49号查询
            if (lock){
            //    7.获取锁成功，查询远程
                log.info("[{}]商品 【缓存未命中，布隆说有，准备回源。。。。】"+skuId);
                SkuDetailTo fromRpc = getSkuDetailFromRpc(skuId);
                //8.数据放缓存
                cacheOpService.saveData(cacheKey,fromRpc);
                //9.解锁
                cacheOpService.unlock(skuId);
                return fromRpc;
            }
        //    10.没有获取到锁
            try {Thread.sleep(1000);
                return cacheOpService.getCacheData(cacheKey,SkuDetailTo.class);
